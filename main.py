...
def run_kivy_app() -> None:
    ...
    class NodeScreen(BoxLayout):
        def __init__(self, **kwargs: Any) -> None:
            ...
            self.title = Label(text="urirun Android URI node", font_size="20sp", bold=True, size_hint_y=None, height=52)
            self.url_label = Label(text="", font_size="14sp", size_hint_y=None, height=44)
            self.status_label = Label(text="", font_size="13sp", size_hint_y=None, height=44)
            self.start_btn = Button(text="Start Termux urirun node", size_hint_y=None, height=56, on_press=self.start_termux)
            self.stop_btn = Button(text="Stop Termux urirun node", size_hint_y=None, height=56, on_press=self.stop_termux)
            self.routes_label = Label(text="", font_size="11sp", halign="left", valign="top")
            for item in (self.title, self.url_label, self.status_label, self.start_btn, self.stop_btn, self.routes_label):
                self.add_widget(item)
            Clock.schedule_interval(self.refresh, 2.0)
            self.refresh(0)

        def start_termux(self, *_: Any) -> None:
            runtime.start_termux()
            self.refresh(0)
        ...
        def refresh(self, _dt: float) -> None:
            ...
            self.url_label.text = "URI API: " + status["url"]
            termux = status["termux"]
            self.status_label.text = "Termux: " + ("running" if termux["running"] else "stopped")
            self.routes_label.text = "\n".join(route["uri"] for route in ROUTES)
